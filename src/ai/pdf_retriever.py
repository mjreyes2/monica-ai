...
class PDFRetriever:
    def __init__(self, index_dir: str | Path = None, source_root: Optional[str | Path] = None):
        # Auto-resolve paths relative to project root
        if index_dir is None:
            try:
                from config.settings import config
                _base = Path(str(config.BASE_DIR))
            except Exception:
                _base = Path(".")
            index_dir = _base / "models" / "kb_index" / "books_pdf"
            if source_root is None:
                _src = _base / "data" / "Monica_Knowledge_Base" / "Textbooks"
                if _src.exists():
                    source_root = _src
