...
class TTSDiagnostics:
    ...
    def _check_model_files(self) -> Dict:
        ...
        try:
            ...
            training_root = base_dir / "data" / "training" / "models" / "tts" / "xtts_official_trained" / "run" / "training"
            accent_tune_root = base_dir / "data" / "training" / "models" / "tts" / "xtts_official_trained" / "run" / "accent_tune"
            ...
            voice_dir = base_dir / "data" / "training" / "recordings" / "wake_phrases"
            if voice_dir.exists():
    ...
    def _check_tts_manager_state(self) -> Dict:
        ...
        try:
            ...
            try:
                from audio.tts_manager import _monica_tts_instance, MONICA_TTS_AVAILABLE
    ...
    def _analyze_results(self, results: Dict):
        ...
        if checks.get('tts_manager_state', {}).get('status') == 'fallback':
            ...
            solutions.append({
                'issue': 'Monica TTS Not Loading',
                'steps': [
                    '1. Check that trained XTTS models exist in data/training/models/tts/',
    ...
    def diagnose_slow_response(self) -> str:
        ...
        try:
            ...
            if not torch.cuda.is_available():
                report.append("[X] CUDA not available - XTTS is running on CPU (very slow)")
                report.append("   Solution: Install CUDA-enabled PyTorch")
            else:
                report.append("[OK] GPU available")
        except:
            report.append("[X] PyTorch not installed")

        # Check if using streaming
        if self.tts_manager:
            if self.tts_manager.use_monica_tts:
                report.append("[OK] Monica XTTS enabled (supports streaming)")
            else:
                report.append(f"[WARN] Using {self.tts_manager.engine_type} instead of Monica XTTS")
        ...
        if self.tts_manager and self.tts_manager.enable_anti_barge_in:
            ...
            report.append(f"[WARN] Anti-barge-in adds up to {silence_ms}ms delay waiting for silence")
    ...
    def diagnose_cutoff(self) -> str:
        ...
        if tm.enable_anti_barge_in and tm.stop_on_user_speech:
            report.append("[X] Anti-barge-in is stopping playback when user speech detected")
        ...
        report.append(f"ℹ  Text is split into chunks of max 220 characters")
        report.append(f"ℹ  0.12s pause between chunks (may cause unnatural pauses)")
