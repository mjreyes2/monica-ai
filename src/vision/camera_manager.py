...
class CameraManager:
    ...
    def _open_camera(self) -> bool:
        ...
        Tries the configured camera_index first; if that fails, scans
        indices 0-4 to find any working camera.  Validates each candidate
        by reading a test frame (some virtual cameras open but return None).
        """
        indices_to_try = [self.camera_index] + [i for i in range(5) if i != self.camera_index]
        
        for idx in indices_to_try:
            result = self._try_open_camera_index(idx)
            if result:
                self.camera_index = idx
                return True
        
        print("[CAMERA] No working camera found on indices 0-4")
        return False
    
    def _try_open_camera_index(self, idx: int) -> bool:
        """Try opening a specific camera index through multiple backends."""
        try:
            ...
            for backend_name, backend_flag in backends:
                try:
                    cam = cv2.VideoCapture(idx, backend_flag)
                    if cam.isOpened():
                        # Validate with a test frame (eliminates virtual/ghost devices)
                        ret, frame = cam.read()
                        if ret and frame is not None and frame.size > 0:
                            self.camera = cam
                            print(f"[CAMERA] Camera {idx} opened via {backend_name} in {time.time() - start_time:.2f}s")
    ...
    def _capture_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Store frame as BGR (native OpenCV format)
                # All vision processing uses BGR; GUI converts to RGB only at display time
    ...
    def get_frame(self) -> Optional[np.ndarray]:
        """Get the current camera frame (BGR format — native OpenCV)."""
        with self.frame_lock:
    ...
    def get_frame_bgr(self) -> Optional[np.ndarray]:
        """Get the current camera frame in BGR format (same as get_frame)."""
