...
class MainWindow:
    ...
    def _create_side_toolbar(self):
        ...
        btn_orb = ttk.Button(ar_frame, text="Monica Orb", command=self._toggle_orb)
        btn_orb.pack(fill=tk.X, pady=3)
        ToolTip(btn_orb, "Show Monica's holographic orb.\nSay 'Monica show yourself' to materialize.")
        
        btn_globe = ttk.Button(ar_frame, text="Globe", command=self._toggle_globe_window)
        btn_globe.pack(fill=tk.X, pady=3)
        ToolTip(btn_globe, "Display interactive 3D holographic globe.\nRotate with hand gestures.")
        
        btn_keyboard = ttk.Button(ar_frame, text="⌨Keyboard", command=self._toggle_keyboard)
        btn_keyboard.pack(fill=tk.X, pady=3)
        ToolTip(btn_keyboard, "Show virtual keyboard overlay.\nType by touching keys with your fingertip.")
        
        btn_dial = ttk.Button(ar_frame, text="Dial", command=self._toggle_dial)
        btn_dial.pack(fill=tk.X, pady=3)
        ToolTip(btn_dial, "Show control dial interface.\nRotate with hand gestures to adjust settings.")
    ...
    def _start_wake_word(self):
        ...
        if self.audio.start_wake_word_detection():
            ...
            self.wake_word_btn.config(text="Wake Word Active")
                    ...
                    if self._active_globe == 'nasa':
                        ...
                        if hasattr(self, 'vision_system') and self.vision_system:
                            if hasattr(self.vision_system, 'ar_hologram') and self.vision_system.ar_hologram:
                                ...
                                from monica_ar_hologram_system import HologramType
    ...
    def _toggle_fog_animation(self):
        ...
        def open_fog():
            try:
                import webbrowser
                import os
                alt_path = str(Path(__file__).resolve().parents[3] / "CSS_FOG_ANIMATION" / "index.html")
    ...
    def _toggle_clouds_animation(self):
        ...
        def open_clouds():
            try:
                import webbrowser
                import os
                alt_path = str(Path(__file__).resolve().parents[3] / "clouds-animation-code" / "haven-animation.html")
                if os.path.exists(alt_path):
                    webbrowser.open(f'file:///{alt_path.replace(os.sep, "/")}')
            except Exception as e:
    ...
    def _toggle_plasma_effect(self):
        ...
        def open_plasma():
            try:
                import subprocess
                import os
                plasma_path = str(Path(__file__).resolve().parents[3] / "monica_advanced_plasma_orb.py")
    ...
    def _toggle_starfield(self):
        ...
        def open_starfield():
            try:
                import webbrowser
                import os
                starfield_path = str(Path(__file__).resolve().parents[3] / "animations" / "starfield.html")
    ...
    def _toggle_aurora(self):
        ...
        def open_aurora():
            try:
                import webbrowser
                import os
                aurora_path = str(Path(__file__).resolve().parents[3] / "animations" / "aurora.html")
