...
# CRITICAL: Apply torchaudio patch BEFORE importing torch/torchaudio/speechbrain
torch_patch.patch_torchaudio()
...
try:
    ...
    print("[OK] torchaudio imported successfully")
except Exception as e:
    print(f"[WARN] torchaudio import failed: {e}")
    print("[WARN] SpeechBrain functionality will be limited")
    TORCHAUDIO_AVAILABLE = False
    torchaudio = None
...
try:
    ...
    print(f"[SPEECHBRAIN] SpeechBrain not available: {e}")
...
try:
    ...
    print(f"[CUSTOM-MODEL] Custom model loader not available: {e}")
    MonicaCustomASR = None
    load_monica_custom_model = None
...
class FinalSpeechBrainRecognizer:
    ...
    def _init_language_model(self):
        ...
        try:
            ...
            # Sort by index and convert to lowercase list
            sorted_vocab = {k.lower(): v for k, v in sorted(vocab.items(), key=lambda x: x[1])}
            vocab_list = list(sorted_vocab.keys())
            ...
            print("[KENLM] Language model decoder ready!")
                ...
                if hasattr(self.asr_model, 'transcribe_tensor'):
                    ...
                    try:
                        if TORCHAUDIO_AVAILABLE:
                            ...
                            # Fallback: save using soundfile or numpy
                            import soundfile as sf
                            audio_numpy = audio_tensor.cpu().numpy()
                            sf.write(str(temp_path), audio_numpy, 16000)
            ...
            try:
                ...
                if in_sr != out_sr and in_sr > 0:
                    if TORCHAUDIO_AVAILABLE:
                        ...
                        print(f"[AUDIO-DEBUG] Skipping resample (torchaudio not available): {in_sr}Hz -> {out_sr}Hz")
            except Exception as e:
