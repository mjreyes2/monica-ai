...
try:
    ...
    print(f"[CUSTOM-LOADER] torchaudio import failed: {e}")
...
# Try to import speechbrain with fallback
try:
    import speechbrain as sb
    from speechbrain.inference.ASR import EncoderASR
    HAS_SPEECHBRAIN = True
except Exception as e:
    print(f"[CUSTOM-LOADER] SpeechBrain import failed: {e}")
...
class MonicaCustomASR:
    ...
    def __init__(self, model_path: Path, hparams_file: Path, device="cuda"):
        ...
        """
        if not HAS_SPEECHBRAIN or not HAS_SENTENCEPIECE:
            raise ImportError("SpeechBrain and SentencePiece are required for custom model loading")
