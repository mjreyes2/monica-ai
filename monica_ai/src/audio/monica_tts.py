...
try:
    ...
    HAS_TTS = False
    print(f"[MONICA TTS] Coqui TTS not available: {e}")

try:
    from monica_ai.src.audio.neural_lattice_effects import NeuralLatticeVoice
    HAS_EFFECTS = True
except ImportError:
    HAS_EFFECTS = False
...
class MonicaTTS:
    ...
    USER_VOICE_DIR = TRAINING_ROOT / "recordings" / "wake_phrases"
    LJSPEECH_DIR = TRAINING_ROOT / "monica_tts_training" / "datasets" / "LJSpeech-1.1" / "wavs"
    COMBINED_METADATA = TRAINING_ROOT / "monica_tts_training" / "datasets" / "monica_combined" / "combined_metadata.json"
    
    # Optimized speaker profile (trained on 16,546 samples)
    OPTIMIZED_SPEAKER_PROFILE = TRAINING_ROOT / "monica_tts_training" / "models" / "monica_voice_optimized" / "monica_speaker_profile.pt"
    
    # Feminine voice conditioning (LJSpeech - Linda Johnson)
    FEMININE_VOICE_CONDITIONING = TRAINING_ROOT / "monica_tts_training" / "models" / "xtts_feminine_official" / "feminine_voice_conditioning.pt"

    XTTS_TRAINING_ROOT = TRAINING_ROOT / "monica_tts_training" / "models" / "xtts_official_trained" / "run" / "training"
    XTTS_BASE_FILES_DIR = XTTS_TRAINING_ROOT / "XTTS_v2.0_original_model_files"
    
    # Cache directory for generated audio
    CACHE_DIR = TRAINING_ROOT / "monica_tts_training" / "cache"
    
    # Output directory for saved speech
    OUTPUT_DIR = TRAINING_ROOT / "monica_tts_training" / "output"
    ...
    def _get_finetuned_checkpoint_path(self) -> Optional[Path]:
        ...
        try:
            from monica_ai.src.config.settings import config as _cfg
    ...
    def _lazy_init(self):
        ...
        with self._lock:
            ...
            try:
                ...
                print("\nTip: Run diagnostics with: from monica_ai.src.tts.tts_diagnostics import TTSDiagnostics; TTSDiagnostics().run_full_diagnosis()")
                return False
