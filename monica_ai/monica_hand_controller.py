...
try:
    ...
    print("⚠️ MediaPipe not available for hand detection")
...
class MonicaHandController:
    ...
    def __init__(self):
        ...
        if HAS_MEDIAPIPE:
            self.mp_hands = mp.solutions.hands.Hands(
                ...
                max_num_hands=2,
                min_detection_confidence=0.7,  # Balanced for accuracy
                min_tracking_confidence=0.5,    # Lower for better responsiveness
                model_complexity=0              # Faster model for reduced lag
            )
        ...
        self.show_landmarks = False  # Hidden by default
        self.show_fingertips = True
        self.fingertip_color = (0, 255, 255)  # Cyan/Yellow
        self.fingertip_size = 10  # Larger for better visibility
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, List[HandState]]:
        ...
        """
        if not self.enabled or not HAS_MEDIAPIPE or self.mp_hands is None:
            return frame, []
            ...
            if len(self.rotation_history) >= 5:
                ...
                if avg_rotation > 8:  # Fast clockwise rotation
                    ...
                    if current_time - self.alarm_trigger_time > 2.0:  # Cooldown
                        ...
                        print("[HandController] 🚨 ALARM TRIGGERED by dial rotation!")
        else:
            self.dial_active = False
