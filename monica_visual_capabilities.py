...
def _load_fer():
    """Lazy load FER emotion detector."""
    global _fer_detector, HAS_FER
    if HAS_FER is None:
        try:
            from fer import FER
            _fer_detector = FER
            HAS_FER = True
            print("✅ FER emotion detection available")
        except ImportError:
            HAS_FER = False
    return _fer_detector if HAS_FER else None


def _load_deepface():
    """Lazy load DeepFace - loads AFTER MediaPipe to avoid conflicts."""
    global _deepface, HAS_DEEPFACE
    if HAS_DEEPFACE is None:
        # Ensure MediaPipe is loaded first to avoid TensorFlow conflicts
        _load_mediapipe()
        try:
            from deepface import DeepFace
            _deepface = DeepFace
            HAS_DEEPFACE = True
            print("✅ DeepFace emotion detection available")
        except ImportError:
            HAS_DEEPFACE = False
...
class ThermalVision:
    ...
    def __init__(self):
        ...
        print("✅ Thermal Vision initialized (simulated)")
...
class NightVision:
    ...
    def __init__(self):
        ...
        print("✅ Night Vision initialized")
...
class EmotionDetector:
    ...
    def __init__(self):
        ...
        # Initialize FER if available (preferred - simpler, more reliable)
        if HAS_FER:
            try:
                self.fer_detector = FER(mtcnn=True)
                print("✅ Emotion Detection initialized (FER with MTCNN)")
            except Exception as e:
                print(f"⚠️ FER initialization failed: {e}")
                self.fer_detector = None
        elif HAS_DEEPFACE:
            print("✅ Emotion Detection initialized (DeepFace)")
        else:
            print("⚠️ Emotion Detection using basic mode")
    ...
    def detect_from_frame(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            # Use FER if available (preferred)
            if self.fer_detector is not None:
                ...
                if emotions and len(emotions) > 0:
                    ...
                    if emotion_scores:
                        ...
                        result["confidence"] = emotion_scores[dominant]
                        result["all_emotions"] = emotion_scores
                        result["face_detected"] = True
                        result["face_location"] = (box[0], box[1], box[2], box[3])
                ...
                if analysis:
                    ...
                    result["emotion"] = analysis.get("dominant_emotion", "neutral")
                    result["all_emotions"] = analysis.get("emotion", {})
                    result["face_detected"] = True
                    ...
                    if result["emotion"] in emotions:
                        result["confidence"] = emotions[result["emotion"]] / 100.0
            else:
                # Basic face detection without emotion
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                ...
                if len(faces) > 0:
                    ...
                    result["emotion"] = "detected"  # Can't determine emotion without DeepFace
    ...
    def detect_from_text(self, text: str) -> Dict[str, Any]:
        ...
        if detected_emotions:
            dominant = max(detected_emotions, key=detected_emotions.get)
            return {
                "emotion": dominant,
    ...
    def __init__(self, model_size: str = "n"):
        ...
        if HAS_YOLO:
            try:
                ...
                print(f"✅ Object Detection initialized (YOLOv8{model_size})")
            except Exception as e:
                print(f"⚠️ Object Detection initialization failed: {e}")
        else:
            print("⚠️ Object Detection not available (ultralytics not installed)")
...
class PublicWebcamAccess:
    ...
    def __init__(self):
        ...
        print(f"✅ Public Webcam Access initialized ({len(self.webcam_list)} locations)")
...
class MonicaVisualCapabilities:
    ...
    def __init__(self):
        ...
        print("=" * 60)
        print("✅ All visual capabilities initialized!")
        print("=" * 60 + "\n")
...
if __name__ == "__main__":
    ...
    print("\n✅ Visual capabilities test complete!")
